(function () {
  ...
  function makeDmUserItem(item) {
    ...
    div.innerHTML = `
            <div class="w100">
                <img src="/PHPgram/static/img/profile/${item.opponent.iuser}/${item.opponent.mainimg}" class="profile w30 h30" 
                 onerror="this.onerror=null; this.src='/PHPram/static/img/profile/defaultProfileImg_100.png'">
            </div>
  ...
  function getDmMsgList(idm) {
    ...
    fetch("PHPgram/dm/msglist" + encodeQueryString(msgParam))
      .then((res) => res.json())
      .then((res) => {
        if (res.length > 0) {
